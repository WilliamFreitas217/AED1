...
void transfere(Lista<Comando> &listaDeComandosA, Lista<Comando> &listaDeComandosB, TabelaHashKinojo *&resultado){
	No<Comando> *p = listaDeComandosA.getPrim()->getProx();
	while(p!=NULL){
...
TabelaHashKinojo *comandoInter(AVL<TabelaHashKinojo> TabelasHashKinojo){
	...
	if(comando=="="){
		...
		transfere(listaDeComandosA, listaDeComandosB, R);
		return R;
	}
	TabelaHashKinojo *R = new TabelaHashKinojo();
	return R;
		...
		if(no== NULL){
		    ...
		    int i = funcaoHash(k, aSerPrintada->getTamDaHash());
				if(aSerPrintada[i].getProgramaKinojo()->getRaiz()!=NULL){
					...
					if(achado!=NULL){
            // Lista<Comando> vetorDeComandos;
            // transferePraUmVetor(aSerPrintada[i].getProgramaKinojo()->getRaiz(), vetorDeComandos);
            // vetorDeComandos.Mostra();
